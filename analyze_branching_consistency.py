...
def analyze_mahalanobis_rank_consistency_per_class(
    sampler,
    device,
    num_samples=10,
    num_branches=8,
        ...
        for class_idx in class_range:
            ...
            for sample_idx in tqdm(
                ...
                for rank in range(1, num_branches + 1):
                    ...
                    rank_results[rank]["mahalanobis_scores"].append(mahalanobis_score)
                    rank_results[rank]["trajectories"].append(trajectory)
                    rank_results[rank]["final_samples_by_class"][class_idx].append(
                        x.cpu()
                    )  # Store by class
    ...
    # Now calculate real FID scores for each rank and class
    print("\n===== Computing Per-Class Real FID Scores =====")
    ...
    for class_idx in class_range:
        print(f"\nProcessing real FID for class {class_idx}...")

        # Get real images for this class
        class_indices = [
            i for i, (_, label) in enumerate(cifar10) if label == class_idx
        ]
        if len(class_indices) > 1000:  # Limit to 1000 real images per class
        ...
        for rank in range(1, num_branches + 1):
            ...
            final_samples = torch.cat(
                rank_results[rank]["final_samples_by_class"][class_idx], dim=0
            )
    ...
    print("\n===== Average Real FID Rank Correlation =====")
    print(
    ...
    for class_idx in class_range:
        ...
        if len(ranks) > 1:
            corr, p_val = spearmanr(ranks, class_fid_values)
            class_correlations[class_idx] = corr
            class_p_values[class_idx] = p_val
        else:
            class_correlations[class_idx] = float("nan")
            class_p_values[class_idx] = float("nan")

    print("\n===== Per-Class Real FID Rank Correlations =====")
    for class_idx in class_range:
        print(
            f"Class {class_idx}: {class_correlations[class_idx]:.4f} (p-value: {class_p_values[class_idx]:.4f})"
        )
    ...
    return {
        ...
        "rank_avg_mahalanobis": rank_avg_mahalanobis,
        "rank_std_mahalanobis": rank_std_mahalanobis,
        "real_fid_scores_by_class": real_fid_scores_by_class,
        "avg_real_fid_scores": avg_real_fid_scores,
        "real_fid_correlation": real_fid_correlation,
        "real_fid_p_value": real_p_value,
        "class_correlations": class_correlations,
        "class_p_values": class_p_values,
        "rank_results": rank_results,
    }
