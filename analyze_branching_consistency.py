...
def analyze_mahalanobis_rank_consistency(
    sampler,
    device,
    num_samples=10,
    num_branches=8,
    ...
    """
    Analyze how consistently selecting the nth-ranked branch by Mahalanobis distance affects final quality.

    This function generates trajectories where at each possible branching point, we consistently
    select the branch ranked n (where n=1 is best, n=2 is second-best, etc.) according
    to the intermediate Mahalanobis distance calculation. We then measure the final quality.
    ...
    Returns:
        Dictionary containing correlation metrics and raw data for plotting
    """
    print("\n===== Mahalanobis Distance Rank Consistency Analysis =====")
                ...
                for rank in range(1, num_branches + 1):
                    ...
                    while t < 1.0 - 1e-6:
                        ...
                        if can_branch:
                            ...
                            # Sort branches by Mahalanobis distance (descending, as higher is better)
                            sorted_indices = torch.argsort(
                                mahalanobis_distances, descending=True
                            )
                    ...
                    # Store results
                    rank_results[rank]["fid_scores"].append(fid_score)
                    rank_results[rank]["mahalanobis_scores"].append(mahalanobis_score)
                    rank_results[rank]["trajectories"].append(trajectory)
                    rank_results[rank]["final_samples"].append(
                        x.cpu()
                    )  # Store for real FID calculation
    ...
    for rank in range(1, num_branches + 1):
        ...
        mahalanobis_scores = rank_results[rank]["mahalanobis_scores"]
    ...
    # Print results for final Mahalanobis distance
    print("\n===== Final Mahalanobis Distance Rank Consistency Results =====")
    print(
        f"Correlation between rank and average final Mahalanobis distance: {rank_mahalanobis_correlation:.4f} (p-value: {mahalanobis_p_value:.4f})"
    )
    print("\nAverage final Mahalanobis distance by rank:")
    for rank in range(1, num_branches + 1):
        print(
    ...
    plt.ylabel("Average Final FID Change Score")
    plt.title("Final FID Change by Consistently Selected Branch Rank (Mahalanobis)")
    plt.grid(True)
    plt.savefig("final_fid_change_mahalanobis_rank_consistency.png")
    plt.close()

    # Plot results for final Mahalanobis distance
    plt.figure(figsize=(10, 6))
    plt.errorbar(
        ranks,
        avg_mahalanobis,
        yerr=[rank_std_mahalanobis[r] for r in ranks],
        fmt="o-",
        capsize=5,
    )
    plt.xlabel("Branch Rank by Intermediate Mahalanobis Distance (1 = best)")
    plt.ylabel("Average Final Mahalanobis Distance")
    plt.title("Final Mahalanobis Distance by Consistently Selected Branch Rank")
    plt.grid(True)
    plt.savefig("final_mahalanobis_rank_consistency.png")
    plt.close()
    ...
    return {
        ...
        "rank_avg_intermediate_fid": rank_avg_fid,
        "rank_std_intermediate_fid": rank_std_fid,
        "rank_avg_mahalanobis": rank_avg_mahalanobis,
        "rank_std_mahalanobis": rank_std_mahalanobis,
        "real_fid_scores": real_fid_scores,
        "real_fid_correlation": real_fid_correlation,
        "real_fid_p_value": real_p_value,
...
def analyze_mahalanobis_rank_consistency_global(
    sampler,
    device,
    num_samples=10,
    num_branches=8,
    ...
    Returns:
        Dictionary containing correlation metrics and raw data for plotting
    """
    print("\n===== Global Mahalanobis Distance Rank Consistency Analysis =====")
                ...
                for rank in range(1, num_branches + 1):
                    ...
                    while t < 1.0 - 1e-6:
                        ...
                        if can_branch:
                            ...
                            mahalanobis_distances = (
                                sampler.batch_compute_mean_difference(aligned_samples)
                            )
                    ...
                    mahalanobis_score = sampler.batch_compute_mean_difference(x).item()
    ...
    # Print results for final FID change
    print("\n===== Final Global FID Change Rank Consistency Results =====")
    print(
        f"Correlation between rank and average final global FID change: {rank_fid_correlation:.4f} (p-value: {p_value:.4f})"
    )
    print("\nAverage final global FID change by rank:")
    for rank in range(1, num_branches + 1):
        print(f"  Rank {rank}: {rank_avg_fid[rank]:.4f} ± {rank_std_fid[rank]:.4f}")

    # Print results for final Mahalanobis distance
    print("\n===== Final Global Mahalanobis Distance Rank Consistency Results =====")
    print(
        f"Correlation between rank and average final global Mahalanobis distance: {rank_mahalanobis_correlation:.4f} (p-value: {mahalanobis_p_value:.4f})"
    )
    print("\nAverage final global Mahalanobis distance by rank:")
    for rank in range(1, num_branches + 1):
        print(
            f"  Rank {rank}: {rank_avg_mahalanobis[rank]:.4f} ± {rank_std_mahalanobis[rank]:.4f}"
        )
    ...
    plt.xlabel("Branch Rank by Intermediate Global Mahalanobis Distance (1 = best)")
    plt.ylabel("Average Final Global FID Change Score")
    plt.title("Final Global FID Change by Consistently Selected Branch Rank")
    plt.grid(True)
    plt.savefig("final_global_fid_change_mahalanobis_rank_consistency.png")
    plt.close()
    ...
    plt.xlabel("Branch Rank by Intermediate Global Mahalanobis Distance (1 = best)")
    plt.ylabel("Average Final Global Mahalanobis Distance")
    plt.title("Final Global Mahalanobis Distance by Consistently Selected Branch Rank")
    plt.grid(True)
    plt.savefig("final_global_mahalanobis_rank_consistency.png")
    plt.close()
    ...
    plt.title("Real FID Score by Consistently Selected Branch Rank")
    plt.grid(True)
    plt.savefig("real_fid_global_mahalanobis_rank_consistency.png")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig("fid_components_by_global_mahalanobis_rank.png")
    plt.close()

    # Return results with added components
    return {
        "intermediate_global_mahalanobis_correlation": rank_fid_correlation,
        "intermediate_global_mahalanobis_p_value": p_value,
        "rank_avg_intermediate_global_fid": rank_avg_fid,
        "rank_std_intermediate_global_fid": rank_std_fid,
        "rank_avg_global_mahalanobis": rank_avg_mahalanobis,
        "rank_std_global_mahalanobis": rank_std_mahalanobis,
        "real_fid_scores": real_fid_scores,
        "real_fid_correlation": real_fid_correlation,
        "real_fid_p_value": real_p_value,
        "fid_mean_components": fid_mean_components,
