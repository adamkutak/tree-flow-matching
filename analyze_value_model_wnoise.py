...
def analyze_value_model_predictions_with_noise(
    sampler,
    device,
    num_samples=100,
    num_branches=8,
    ...
    """
    Analyze how well the value model predicts actual FID differences using noise-based branching.
    ...
    Args:
        ...
        noise_scale: Base scale for noise when creating branches
    ...
    """
    print("\n===== Value Model Prediction Analysis (Noise-Based Branching) =====")
        ...
        for class_idx in class_range:
            ...
            for batch_idx in tqdm(
                ...
                for i in range(actual_batch_size):
                    ...
                    current_time = current_times[i].item()

                    # Create branches for this sample by adding noise
                    ...
                    # Add noise to create branches - scale noise by (1-t) like in batch_sample
                    # This makes noise smaller as we approach t=1
                    adaptive_noise_scale = noise_scale * (1 - current_time)
                    perturbations = torch.randn_like(branches) * adaptive_noise_scale
                    branched_samples = branches + perturbations

                    # Now simulate all branches forward one step to a common time point
                    # Use the next timestep in the schedule after the branch point
                    next_timestep = sampler.timesteps[branch_step + 1].item()
                    dt = next_timestep - current_time

                    # Get velocity for all branches in a single batched calculation
                    velocity = sampler.flow_model(
                        branch_times, branched_samples, branch_labels
                    )

                    # Apply the step to all branches at once
                    aligned_samples = branched_samples + velocity * dt
                    ...
                    next_velocity = sampler.flow_model(
                        aligned_times, aligned_samples, branch_labels
                    )
                    lookahead_samples = (
                        aligned_samples + next_velocity * dt_to_end.view(-1, 1, 1, 1)
                    )
                    ...
                    for j in range(num_branches):
                        # Negate to match the value model's convention (higher is better)
...
def main():
    ...
    analysis_results = analyze_value_model_predictions_with_noise(
        sampler=sampler,
        device=device,
        num_samples=2000,
        num_branches=16,
