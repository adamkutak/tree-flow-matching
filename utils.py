...
def sample_spherical_simplex_gaussian(batch_size, channels, height, width, device):
    """
    Sample Gaussian noise on a high-dimensional spherical simplex such that samples
    within a batch are maximally far apart from each other.

    This uses the property that on a high-dimensional sphere, points can be made
    maximally separated by using a regular simplex construction.

    Args:
        batch_size: Number of samples in the virtual batch
    ...
    Returns:
        torch.Tensor: [batch_size, channels, height, width] with maximally separated noise
    """
    total_dim = channels * height * width

    if batch_size == 1:
        # Single sample, just return normal Gaussian
        return torch.randn(1, channels, height, width, device=device)

    # For batch_size > 1, we create maximally separated points on the sphere
    # Use Gram-Schmidt orthogonalization to create an orthogonal basis

    # Start with random vectors
    vectors = torch.randn(batch_size, total_dim, device=device)
    ...
    for i in range(batch_size):
        ...
        for j in range(i):
            proj = torch.dot(vec, orthogonal_vectors[j]) * orthogonal_vectors[j]
            vec = vec - proj

        # Normalize the vector
        vec_norm = torch.linalg.vector_norm(vec)
        if vec_norm > 1e-8:  # Avoid division by zero
            orthogonal_vectors[i] = vec / vec_norm
        else:
            # If we run out of orthogonal directions, use random normalized vector
            orthogonal_vectors[i] = torch.randn(total_dim, device=device)
            orthogonal_vectors[i] = orthogonal_vectors[i] / torch.linalg.vector_norm(
                orthogonal_vectors[i]
            )

    # Scale to unit sphere (they should already be normalized, but ensure it)
    orthogonal_vectors = orthogonal_vectors / torch.linalg.vector_norm(
        orthogonal_vectors, dim=1, keepdim=True
    )

    # Reshape back to image dimensions
    return orthogonal_vectors.view(batch_size, channels, height, width)
