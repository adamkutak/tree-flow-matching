    ...
    def train(self, train_loader, n_epochs=3):
        ...
        for epoch in range(n_epochs):
            ...
            for batch_idx, (x1, y) in enumerate(pbar):
                ...
                noise_amounts = torch.rand(x1.shape[0], device=self.device)  # One noise per image
                noisy_x1 = x1 + torch.randn_like(x1) * noise_amounts.view(-1, 1, 1, 1)
                predicted_noise = self.policy_model(
            ...
            for idx in epoch_flow_losses:
                if len(epoch_flow_losses[idx]) > 0:
                    ...
                    print(f"  {noise_levels[idx]:.5f}->{noise_levels[idx+1]:.5f} "
    ...
    def sample(
        ...
        with torch.no_grad():
            ...
            for step in pbar:
                ...
                current_sigma = torch.full((len(current_samples),), self.noise_levels[step], device=self.device)
                traj = torchdiffeq.odeint(
                    lambda t, x: self.flow_model(t, x, current_labels, sigma=current_sigma),
                    current_samples,
                ...
                # 2. Get policy scores
                noise_estimates = self.policy_model(
                ...
                if step < num_steps - 1:
                    next_sigma = torch.tensor(self.noise_levels[step + 1], device=self.device)
                    noise_scale = current_sigma[0] - next_sigma  # Use any sigma since they're all the same
                    
                    # Expand samples and add noise
                ...
                #     grid = make_grid(current_samples[:8], nrow=8, normalize=True)
                #     plt.imshow(grid.cpu().permute(1, 2, 0))
                #     plt.title(f'Step {step}, Samples: {len(current_samples)}, Avg Noise Est: {noise_estimates.mean():.4f}')
                #     plt.axis('off')
                #     plt.show()
